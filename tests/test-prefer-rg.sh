...
assert_denied 'find . | xargs -n1 grep foo' "xargs with flags then grep"
assert_denied 'cat file|grep foo' "piped grep without spaces"
assert_denied 'ls;grep foo file' "semicolon grep without spaces"
...
assert_allowed 'rg prefer-grep file.txt' "unquoted arg containing grep (hyphen)"
assert_allowed 'rg foo grep-results/' "unquoted path arg containing grep"
assert_allowed 'ls docs/grep.md' "path containing grep (slash+dot)"
assert_allowed 'cat /usr/bin/grep' "path to grep binary"
assert_allowed 'ls # grep here' "grep inside shell comment"
assert_allowed 'rg --glob=*.grep foo' "flag value containing grep"
